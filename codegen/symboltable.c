...
Ty_ty S_vardeclaration(A_vardeclaration vardeclaration)
{
	...
	if(vardeclaration->kind == Typespecifier2vardeclaration)
	{
		...
		s = S_insert(t, vardeclaration->u.typespecifier2vardeclaration.id.tokenvalue);
	...
	else if(vardeclaration->kind == TypespecifierNUM2vardeclaration)
	{
		...
		t = Ty_Array(t);
		s = S_insert(t, vardeclaration->u.typespecifierNUM2vardeclaration.id.tokenvalue);

		if(scope == 0)	// global
		{
			s->ac_register = GP;
			s->offset = globalsize;
			globalsize += vardeclaration->u.typespecifierNUM2vardeclaration.size;
		}
		else	// local
		{
			s->ac_register = FP;
			s->offset = stacksize;
			stacksize += vardeclaration->u.typespecifierNUM2vardeclaration.size;
		}
