...
A_vardeclaration	F_typespecifierNUM2vardeclaration(A_typespecifier typespecifier, char *id, char *size)
{
	...
	s->u.typespecifierNUM2vardeclaration.size.tokennumber = NUM;
	s->u.typespecifierNUM2vardeclaration.size.tokenvalue = malloc(sizeof(char) * (strlen(size) + 1));
	memset(s->u.typespecifierNUM2vardeclaration.size.tokenvalue, 0, strlen(size) + 1);
	memcpy(s->u.typespecifierNUM2vardeclaration.size.tokenvalue, size, strlen(size));
...
{
	A_relop s = malloc(sizeof(*s));

	s->relop.tokennumber = tokennumber;
	s->relop.tokenvalue = malloc(sizeof(char) * (strlen(relop) + 1));
	memset(s->relop.tokenvalue, 0, strlen(relop));
	memcpy(s->relop.tokenvalue, relop, strlen(relop));
...
{
	A_addop s = malloc(sizeof(*s));

	s->addop.tokennumber = tokennumber;
	s->addop.tokenvalue = malloc(sizeof(char) * (strlen(addop) + 1));
	memset(s->addop.tokenvalue, 0, strlen(addop) + 1);
	memcpy(s->addop.tokenvalue, addop, strlen(addop));
...
{
	A_mulop s = malloc(sizeof(*s));

	s->mulop.tokennumber = tokennumber;
	s->mulop.tokenvalue = malloc(sizeof(char) * (strlen(mulop) + 1));
	memset(s->mulop.tokenvalue, 0, strlen(mulop) + 1);
	memcpy(s->mulop.tokenvalue, mulop, strlen(mulop));
...
A_factor		F_num2factor(char *num)
{
	...
	s->u.num2factor.num.tokennumber = NUM;
	s->u.num2factor.num.tokenvalue = malloc(sizeof(char) * (strlen(num) + 1));
	memset(s->u.num2factor.num.tokenvalue, 0, strlen(num) + 1);
	memcpy(s->u.num2factor.num.tokenvalue, num, strlen(num));
